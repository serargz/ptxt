...
func (c CalendarGoogle) fetchEvents(t time.Time) ([]CalendarEvent, error) {
	events, err := c.calService.Events.List("primary").ShowDeleted(false).
		SingleEvents(true).TimeMin(t.Format(time.RFC3339)).MaxResults(10).OrderBy("startTime").Do()
